...
class _FakeMarketHandler:
    ...
    def fetch_domestic_future_price(self, shtn_pdno: str, market_cls_code: str = "MKI") -> int:
        self.calls.append(("fetch_domestic_future_price", (shtn_pdno,), {"market_cls_code": market_cls_code}))
        return 345.0

    def create_futureoption_buy_order(self, shtn_pdno: str, quantity: int, account_product_code: str = "03"):
        self.calls.append(
            (
    ...
    def create_futureoption_sell_order(self, shtn_pdno: str, quantity: int, account_product_code: str = "03"):
        self.calls.append(
            (
...
def test_kospi_futures_live_mode_rejects_simulation_handler():
    ...
    try:
        ...
        raise AssertionError("expected live mode validation to reject simulation handler")
...
def test_chart_master_kospi_signal_uses_futures_handler_and_budget(tmp_path):
    ...
    anyio.run(trader._handle_chart_master_kospi_signal, _build_signal("buy_candidate", "코스피 2계약 매수진입"))

    state = json.loads(trader.kospi_futures_state_path.read_text(encoding="utf-8"))
    assert state["open_quantity"] == 1
    assert any(call[0] == "create_futureoption_buy_order" for call in trader.market_handler.calls)
    buy_call = next(call for call in trader.market_handler.calls if call[0] == "create_futureoption_buy_order")
    assert buy_call[1] == ("101W09", 1)
    assert buy_call[2]["account_product_code"] == "03"

    anyio.run(trader._handle_chart_master_kospi_signal, _build_signal("sell", "청산하겠습니다"))

    state = json.loads(trader.kospi_futures_state_path.read_text(encoding="utf-8"))
    assert state["open_quantity"] == 0
    assert any(call[0] == "create_futureoption_sell_order" for call in trader.market_handler.calls)
...
def test_chart_master_kospi_blocks_repeated_buys_after_insufficient_orderable_amount(tmp_path):
    ...
    buy_calls = [call for call in trader.market_handler.calls if call[0] == "create_futureoption_buy_order"]
    assert len(buy_calls) == 1

    state = json.loads(trader.kospi_futures_state_path.read_text(encoding="utf-8"))
    assert state["buy_blocked_date"] == "2026-06-30"
    assert state["buy_blocked_reason"] == "모의투자 주문가능금액이 부족합니다."
    assert any("KOSPI Futures Budget Mismatch" in message for message in trader.notifier.messages)
...
def test_chart_master_kospi_track_only_updates_paper_position_and_pnl(tmp_path):
    ...
    assert not any(call[0] == "create_futureoption_buy_order" for call in trader.market_handler.calls)
