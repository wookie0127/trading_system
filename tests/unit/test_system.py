...
from src.trading_system.snapshots.schemas import MarketSnapshot, PriceInfo, TechnicalInfo, HigherTimeframeInfo
...
def test_position_state_machine():
    ...
    assert PositionStateMachine.is_valid_transition("LONG", "OPEN_SHORT") is False  # Immediate reversal invalid
...
def test_stop_loss_calculator():
    sl_long = StopLossCalculator.calculate_stop_loss("LONG", entry_price=60000.0, atr_14=1000.0, multiplier=2.0)
    assert sl_long == 58000.0

    sl_short = StopLossCalculator.calculate_stop_loss("SHORT", entry_price=60000.0, atr_14=1000.0, multiplier=2.0)
    assert sl_short == 62000.0


def test_kill_switch_trigger():
    ks = KillSwitch(max_daily_loss_ratio=0.02, max_drawdown_ratio=0.08)
    assert ks.check(daily_loss_ratio=0.01, drawdown_ratio=0.05, consecutive_losses=2) is False
    assert ks.check(daily_loss_ratio=0.025, drawdown_ratio=0.05, consecutive_losses=2) is True
