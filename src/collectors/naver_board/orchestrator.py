...
async def collect_board_task(symbols: list[str], max_pages: int):
    ...
    logger = get_run_logger()
    logger.info(
        f"Starting collection for {len(symbols)} symbols (max_pages={max_pages})"
    )
...
async def naver_board_flow(symbols: str = DEFAULT_SYMBOLS, max_pages: int = MAX_PAGES):
    ...
    try:
        ...
        await notifier.notify_all(
            f"❌ *[Naver Board Flow]* 실행 중 오류 발생: {str(e)}"
        )
        raise e
...
if __name__ == "__main__":
    ...
    # 배포: prefect deploy src/collectors/naver_board/orchestrator.py --name "Naver-Board-Sync" --cron "0 */12 * * 1-5"
