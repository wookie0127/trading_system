...
def handle_backfill_message(message, say, context, logger):
    try:
        ...
        say(
            f"♻️ {stock_name}({code})의 과거 30일 1분봉 백필을 시작합니다. (이미 수집된 날짜는 건너뜁니다)"
        )
...
def handle_balance_message(message, say, logger):
    try:
        ...
        is_success = balance.get("rt_cd") == "0" or (
            balance.get("output2") and len(balance.get("output2", [])) > 0
        )
        ...
        if is_success:
            ...
            error_msg = (
                balance.get("msg1")
                or balance.get("error_description")
                or "알 수 없는 오류"
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info(
            f"Bot connected as: {auth_test['user']} (ID: {auth_test['user_id']})"
        )
    except Exception as e:
