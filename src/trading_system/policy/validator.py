...
class PolicyValidator:
    @staticmethod
    def validate_decision(decision: TradingDecision, snapshot: MarketSnapshot) -> Tuple[bool, str]:
        """
        ...
        if not PositionStateMachine.is_valid_transition(current_state, action):
            return False, f"Action {action} is invalid for current position state {current_state}"

        # Prohibit OPEN when confidence is zero or negative
        if action in ["OPEN_LONG", "OPEN_SHORT"] and decision.confidence <= 0.0:
            return False, f"Cannot open position with zero or negative confidence: {decision.confidence}"
