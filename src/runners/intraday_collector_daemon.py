...
from core.kis_config import API_ROOT
from collectors.kr.kospi200_component_collector import load_components
...
RANDOM_DELAY_RANGE = (
    0.2,
    0.8,
)  # Random sleep (seconds) between individual stock requests
...
async def fetch_latest_candles(
    client: httpx.AsyncClient, auth: KISAuthHandler, symbol: str
) -> pd.DataFrame:
        ...
        if _is_market_open():
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"Component file for {now.date()} not found. Fetching now..."
                    )
                    from collectors.kr.kospi200_component_collector import (
                        collect_kospi200_components,
                    )
                ...
                logger.info(
                    f"Finished cycle. Waiting {wait_time / 60:.1f} minutes until next..."
                )
