...
def run_backtest(df: pl.DataFrame, initial_capital: float = 10000.0) -> pl.DataFrame:
    ...
    res_df = res_df.with_columns([
        (pl.col("position").shift(1).fill_null(0) * pl.col("returns")).alias("strategy_returns")
    ])
...
def print_performance(df: pl.DataFrame, initial_capital: float = 10000.0):
    ...
    df = df.with_columns([
        pl.col("cumulative_returns").cum_max().alias("cum_max")
    ])
    df = df.with_columns([
    ...
    win_rate = (positive_trades / (positive_trades + negative_trades) * 100) if (positive_trades + negative_trades) > 0 else 0
