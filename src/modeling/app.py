...
def load_data(symbol, period, interval):
    ...
    if interval == "1m" and period not in ["1d", "5d", "7d"]:
        period = "7d"
        st.warning(f"1m interval is limited to 7d. Adjusting period to 7d.")
...
if df_full.height > 0:
    ...
    if col1.button("Next Step ➡️"):
        st.session_state.days_to_show = min(st.session_state.days_to_show + step, total_len)
    ...
    # Price & Position
    fig.add_trace(go.Scatter(x=results["Datetime"] if "Datetime" in results.columns else results["Date"], 
                             y=results["Close"], name="Price", line=dict(color='white')), row=1, col=1)
    
    # Long Signals
    longs = results.filter(pl.col("long_signal") == True)
    if longs.height > 0:
        fig.add_trace(go.Scatter(x=longs["Datetime"] if "Datetime" in longs.columns else longs["Date"], 
                                 y=longs["Close"], mode='markers', name='Long Entry',
                                 marker=dict(symbol='triangle-up', size=10, color='lime')), row=1, col=1)
    
    # Short Signals
    shorts = results.filter(pl.col("short_signal") == True)
    if shorts.height > 0:
        fig.add_trace(go.Scatter(x=shorts["Datetime"] if "Datetime" in shorts.columns else shorts["Date"], 
                                 y=shorts["Close"], mode='markers', name='Short Entry',
                                 marker=dict(symbol='triangle-down', size=10, color='red')), row=1, col=1)

    # MACD
    fig.add_trace(go.Bar(x=results["Datetime"] if "Datetime" in results.columns else results["Date"], 
                         y=results["macd_hist"], name="MACD Hist"), row=2, col=1)
    
    # Squeeze Momentum
    # marker_color 계산 시 return_dtype을 명시하거나 pl.when 사용 (에러 방지)
    momentum_colors = results.with_columns(
        pl.when(pl.col("momentum") > 0).then(pl.lit("cyan")).otherwise(pl.lit("orange")).alias("m_color")
    )["m_color"]

    fig.add_trace(go.Bar(x=results["Datetime"] if "Datetime" in results.columns else results["Date"], 
                         y=results["momentum"], name="Squeeze Momentum",
                         marker_color=momentum_colors), row=3, col=1)
    ...
    fig_ret = go.Figure()
    fig_ret.add_trace(go.Scatter(x=results["Datetime"] if "Datetime" in results.columns else results["Date"], 
                                 y=results["cumulative_returns"], name="Strategy", line=dict(color='lime')))
    
    # Buy & Hold 비교용
    bh_ret = (results["Close"] / results["Close"][0])
    fig_ret.add_trace(go.Scatter(x=results["Datetime"] if "Datetime" in results.columns else results["Date"], 
                                 y=bh_ret, name="Buy & Hold", line=dict(color='gray', dash='dash')))
