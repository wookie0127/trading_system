...
async def backfill_daily_task(symbol: str, years: int):
    ...
    limit = years * 260
    logger.info(
        f"Starting {years}-year daily data backfill for {symbol} (limit={limit})"
    )
...
async def backfill_flow(symbol: str, daily_years: int = 5, intraday_days: int = 30):
    ...
    await notifier.notify_all(
        f"⏳ *[Backfill]* `{symbol}` 종목의 데이터 백필(일봉 {daily_years}년, 1분봉 {intraday_days}일)을 시작합니다."
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--years", type=int, default=5, help="수집할 일봉 과거 연수 (기본 5년)"
    )
    parser.add_argument(
        "--days", type=int, default=30, help="수집할 1분봉 과거 일수 (최대 약 30일)"
    )
