...
from collectors.us.yahoo_finance_collector import collect_kospi200_intraday
from collectors.kr.kospi200_component_collector import fetch_kospi200_components
from storage.parquet_writer import daily_path
...
async def main():
    ...
    for i in range(31):  # 0 to 30 inclusive
        ...
        if dest_file.exists():
            logger.info(
                f"Data for {target_date} already exists at {dest_file}. Skipping."
            )
            continue
