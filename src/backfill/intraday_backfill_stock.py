...
from collectors.us.yahoo_finance_collector import _to_yf_kr, _download_1min
...
async def backfill_stock_intraday(symbol_raw: str, days: int = 30) -> dict:
    ...
    logger.info(
        f"Starting 1-min backfill for {symbol_raw} ({yf_symbol}) for the last {days} days..."
    )
    ...
    for i in range(days):
        ...
        try:
            df = await asyncio.to_thread(
                _download_1min, [yf_symbol], target_date, target_date
            )
            if not df.empty:
                ...
                logger.warning(
                    f"  No data for {target_date} (possibly too old for 1-m or holiday)"
                )
                results["failed"] += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Number of days to go back (max ~30 for 1-m)",
    )
