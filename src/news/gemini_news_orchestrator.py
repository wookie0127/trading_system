...
async def _fetch_news_items(
    feed_urls: list[str], max_items: int, logger
) -> list[FeedItem]:
...
async def daily_news_summary_gemini_flow(
    ...
    resolved_root = obsidian_root or os.getenv(
        "OBSIDIAN_VAULT_DIR", str(DEFAULT_OBSIDIAN_ROOT)
    )
    resolved_subdir = obsidian_subdir or os.getenv(
        "NEWS_OBSIDIAN_SUBDIR", DEFAULT_OBSIDIAN_SUBDIR
    )
    resolved_max_items = max_items or int(
        os.getenv("NEWS_MAX_ITEMS", str(DEFAULT_MAX_ITEMS))
    )
    resolved_model = gemini_model or os.getenv("GEMINI_MODEL", DEFAULT_GEMINI_MODEL)
    resolved_timeout = timeout_seconds or int(
        os.getenv("GEMINI_TIMEOUT_SECONDS", str(DEFAULT_TIMEOUT_SECONDS))
    )
