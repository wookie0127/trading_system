...
def build_obsidian_markdown(
    ...
    return f"{normalized}\n\n---\n*Generated by Gemini API at {generated_at} ({timezone_label})*\n"
...
@task(name="Run Gemini API", retries=2, retry_delay_seconds=60)
async def run_gemini_task(prompt: str, model: str, timeout_seconds: int) -> str:
    logger = get_run_logger()
    logger.info(f"Running Gemini API with model={model or 'default'}")
...
async def daily_news_summary_gemini_flow(
    ...
    markdown_text = await run_gemini_task(prompt, resolved_model, resolved_timeout)
