...
def run_historical_replay(max_cycles: int = 20, db_output_path: str = "trading_system.db"):
    ...
    logger.info(f"Starting Historical Replay loop from candle index {start_idx} to {end_idx}...")
    ...
    for i in range(start_idx, end_idx):
        ...
        logger.info(f"[{i - start_idx + 1}/{end_idx - start_idx}] Replaying candle at {candle_ts}...")
        ...
        if action == "NO_TRADE":
            ...
            success_count += 1
            logger.info(f"Executed action: {action} (Decision ID: {res.get('decision_id')})")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Historical Replay for Gemini Trading System")
    parser.add_argument("--cycles", type=int, default=10, help="Number of 4h candles to replay")
    parser.add_argument("--db", type=str, default="trading_system.db", help="Output database file path")
