...
def download_and_resample_kospi_samples():
    ...
    for code, yf_symbol in symbols.items():
        logger.info(
            f"Downloading historical 1m candles for {code} ({yf_symbol}) from {start_date} to {end_date}..."
        )
        ...
        try:
            ...
            for tf in timeframes:
                ...
                logger.success(
                    f"Saved {resampled.height} candles of {tf} to {out_path}"
                )
...
def pd_concat_yfinance(chunks: list, symbol: str) -> pl.DataFrame:
    ...
    combined_pd = combined_pd.rename(
        columns={
            time_col: "timestamp",
            "Open": "open",
            "High": "high",
