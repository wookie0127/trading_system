...
def run_validation():
    ...
    if not os.path.exists(data_path):
        logger.error(
            f"Processed dataset not found at {data_path}. Run binance_collector and resampler first."
        )
        return
    ...
    print("\n" + "=" * 60)
    print(" BACKTEST VALIDATION REPORT: BTCUSDT (1-Hour timeframe)")
    print("=" * 60)
    ...
    for label, key, fmt in metrics_to_show:
        ...
        opt_str = (
            fmt.format(opt_val) if isinstance(opt_val, (int, float)) else str(opt_val)
        )
